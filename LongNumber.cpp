...
void			ft_lnum_make_decimal(t_longnumber **lnum, int shift)
{
	...
	big_offset = FT_LONGNUMBER_SIZE -
		(length / FT_LONGNUMBER_DIG_ON_INT + (length % FT_LONGNUMBER_DIG_ON_INT == 0 ? 0 : 1)) -
		(shift / FT_LONGNUMBER_DIG_ON_INT + (shift % FT_LONGNUMBER_DIG_ON_INT == 0 ? 0 : 1));
	min_offset = 2 * FT_LONGNUMBER_DIG_ON_INT - (shift % FT_LONGNUMBER_DIG_ON_INT)
		- (length % FT_LONGNUMBER_DIG_ON_INT);
...
int main()
{
	...
	lnum2 = ft_lnum_new_int(10);
	lnum3 = ft_lnum_pow(lnum1, lnum2, 0, 0);
	ft_lnum_make_decimal(&lnum3, 3);
	lnum4 = ft_lnum_pow(lnum1, lnum2, 0, 0);
	ft_lnum_make_decimal(&lnum4, 3);
	lnum3 = ft_lnum_add(lnum3, lnum4, 1, 0);
